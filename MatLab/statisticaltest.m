function [h,p,irrelevantes,normais] = statisticaltest(classe1, classe2)
% Executa um teste estatistico simples para detectar caracteristicas
% irrelevantes na distincao de duas classes.
% INPUT:
% - classe1 = matriz L x N1 (caracteristicas x padroes)
% - classe2 = matriz L x N2 (caracteristicas x padroes)
% OUTPUT:
% - h: 1 x L - rejeicao da hipotese nula (1 = rejeita, 0 = nao rejeita)
% - p: 1 x L - p-values da comparacao
% - irrelevantes: caracteristicas irrelevantes
% - normais: caracteristicas que passaram no teste de normalidade sw

% NECESSITA DO TOOLBOX DE STATISTICA!

if ~isequal(size(classe1,1),size(classe2,1))
...
for i=1:L 
    n=0;
    [Hn]=swtest(classe1(i,:)); %testando normalidade das classes
    if ~Hn
        [Hn]=swtest(classe2(i,:)); %testando normalidade das classes
        if ~Hn %se sao normais: ttest
            n=1; %normais
        else
           fprintf('Hipotese de normalidade rejeitada para a caracteristica %d da Classe 2.\n',i) 
        end
    else
       fprintf('Hipotese de normalidade rejeitada para a caracteristica %d da Classe 1.\n',i) 
    end
    if n
        [h(i),p(i)]=ttest2(classe1(i,:),classe2(i,:));
        normais=[normais,i];
    else
        fprintf('Chamando RankSum para caracteristica %d.\n',i) 
        [p(i),h(i)]=ranksum(classe1(i,:),classe2(i,:));        
    end
end
irrelevantes=find(h==0);
