function [alpha, w0, w, evals, stp, glob] = aula34_SVM(X, Y, kernel, kpar1, kpar2, C, tol, steps, eps, method)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Essa funcao implementa o algoritmo "SMO2" de John C. Platt para treinar
%   uma SVM entre duas classes.
%   (Veja: "Sequential Minimal Optimization: A Fast Algorithm 
%   for Training Support Vector Machines": 
%   http://research.microsoft.com/pubs/69644/tr-98-14.pdf)
%   
% O procedimento inclui as modificacoes de Keerthi et al. ("Improvements 
%   to Platt's SMO Algorithm for SVM Classifier Design":
%   http://web.cs.iastate.edu/~honavar/keerthi-svm.pdf)
%
% Autor dessas funcoes: Michael Mavroforakis (c) 2003, Computational
% Biomedicine Lab, University of Houston, Texas.
%
% Observacao: no procedimento de Platt que envolve randomizacao,
% os resultados podem nao ser identicos mesmo no mesmo conjunto de dados.
%
% INPUTS:
% X: matriz N x L: Padroes (N) por Caracteristicas (L)    
% Y: vetor N x 1: valores das classes de cada padrao (+1 ou -1)
% kernel:  Uma das seguintes strings:
%               'linear' : SVM Linear (padrao)
%               'poly' : polinomial
%               'rbf' : gaussiano
%               'sigmoid' : tanh
% kpar1:  primeiro parametro para a funcao de Kernel (opcional, padrao=0)
% kpar2:  segundo parametro para a funcao de Kernel (opcional, padrao=0)
% C:  parametro que regula o trade off entre a largura da margem e o numero
%     de erros de classificacao
% tol: tolerancia de Keerthi (Keerthi usa 0.001)
% steps: numero maximo de iteracoes 
% eps: precisao
% method:  0->Platt, 1->Keerthi 1, 2->Keerthi 2
%
% OUTPUTS:
% alpha:  vetores coluna com os multiplicadores de lagrange, ordenados por 
%        classes (as primeiras N1 linhas correspondem aos N1 pontos da
%        primeira classe, as seguintes N2 linhas correspondem aos N2 pontos
%        da segunda classe)
% w0:     valor de threshold do classificador linear (w^T x + w0)
% w:     a normal ao hiperplano no caso de Kernel Linear (sem valor para os
%        demais Kernels)
% evals: numero de normas calculadas
% stp:   numero de iteracoes efetuadas
% flag:  indica se o treinamento da SVM terminou de maneira anormal (flag =1)
%        ou normal (flag = 0). Final anormal indica que outro Kernel deveria
%        ser selecionado e que resultados podem ser imprecisos.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:n
    K(:,i)=aula34_CalcKernel(X,X(i,:), kernel, kpar1, kpar2);
end
